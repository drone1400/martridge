...
dotnet clean ./sharpcompress/src/SharpCompress/SharpCompress.csproj --configuration Release -p:Platform="x64" --framework net10.0
dotnet clean ./martridge/src/martridge.csproj --configuration Release -p:Platform="x64" --framework net10.0
dotnet publish ./martridge/src/martridge.csproj --output ./publish/net10.0_win-x64-offline/martridge/ --configuration Release -p:Platform="x64" --framework net10.0 --runtime win-x64 --self-contained true -p:PublishSingleFile=true -p:IncludeNativeLibrariesForSelfExtract=true -p:DisableFeatureOnline=true